...
parser.add_argument("--step", type=int, default=400, help="step to test the model performance. Default=2000")
...
def main():
    global opt, logger, model, criterion_L1,criterion_mse,ms_ssim_loss,best_psnr,loss_network
...
# --- according input set max_val : 255 or 1--- #
    ms_ssim_loss =  loss.MS_SSIM(max_val = 1)
...
def train(training_data_loader, indoor_test_loader,optimizer, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        loss = mse_loss + 0.04*loss_network(output, label)
        loss_final = loss
        loss_add.append(loss.data)
        loss_mse.append(mse_loss.data)
        loss_final.backward()
        optimizer.step()
        if iteration % 200 == 0:
            loss_add_mean = sum(loss_add) / len(loss_add)
            loss_mse_mean = sum(loss_mse) / len(loss_mse)
            print("===> Epoch[{}]({}/{}): Loss: {:.6f}".format(epoch, iteration, len(training_data_loader),
                                                             loss_add_mean))
            logger.add_scalar('loss', loss_add_mean, steps)
